...
class BaseGame():
    ...
    def baseInit(self):
        ...
        self.frameSeq = np.zeros(shape=(self.settings.frameSequenceLen,
                                        self.settings.gameRes,
                                        self.settings.gameRes),
                                 dtype=np.float32)
        self.episodeData = []
    ...
    def sendFrameToWorker(self, bootstrap):
        ...
        if (self.timeStep == -1): # Dont want to feed the agent any data not generated by the game, like all black frames
            for i in range(self.settings.frameSequenceLen):
                self.frameSeq[i] = frame
        else:
